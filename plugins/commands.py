...
HOME_TEXT = "<b>Hey  [{}](tg://user?id={}) 😉</b>\n\nIam A Bot Built To Play or Stream Videos In Honey Bee Movies"
admin_filter=filters.create(is_admin) 

@Client.on_message(filters.command(['start', f"start@{Config.BOT_USERNAME}"]))
async def start(client, message):
    if len(message.command) > 1:
        if message.command[1] == 'pleh':
    ...
    buttons = [
        [
            InlineKeyboardButton('🐝 Main Channel', url='https://t.me/honeybeemovies'),
            InlineKeyboardButton('🧩 Updates', url='https://t.me/malluflix')
        ],
        [
            InlineKeyboardButton('🐝 Join', url='https://t.me/honeybeemovies'),
            InlineKeyboardButton('🗑 Close', callback_data='close'),
        ]
    ]
    ...
    if message.chat.type != "private" and message.from_user is None:
        k=await message.reply(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(f"pleh", url=f"https://telegram.dog/{Config.BOT_USERNAME}?start=pleh"),
                    ]
                ]
            ),)
        await delete_messages([message, k])
...
async def repo_(client, message):
    await message.reply("Pong", disable_web_page_preview=True)
    await delete_messages([message])
